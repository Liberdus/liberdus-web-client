<template>
  ...
  <div>
    <a-card title="Verify Email" style="max-width: 600px; margin:auto">
      ...
      <div v-else-if="!checkingCode && codeAccepted" class="checkingCode-container">
      ...
      <div v-else class="email-container">
        <form class="email-form">
          ...
          <div class="email-input-container">
            ...
            <a-input v-model="code" type="text" placeholder="Verification code" size="large" />
          </div>

          <div class="error-message-container">
            <p v-if="!$v.code.integer" class="input-error-message">
              Code must be digits only
            </p>
            <p v-else-if="code.length > 0 && !$v.code.minLength" class="input-error-message">
              Required at least 6 digits
            </p>
          </div>

          <a-button @click="onVerifyCode" :disabled="!isCodeValid" size="large" shape="round" type="primary">Submit Verification Code </a-button>
        </form>
      </div>
    </a-card>
  </div>
...
import { required, minLength, integer, between, email } from 'vuelidate/lib/validators';
Vue.use(VueOnsen);
Vue.use(Vuelidate);
  ...
  methods: {
    async onVerifyCode(e) {
      ...
      if (isSubmitted) {
        ...
        setTimeout(function() {
          ...
          clearInterval(this.codeChecker);
          this.codeChecker = null;
          this.error = 'Verification code is not approved within 15 seconds. Please try again.';
        }, 15000);
    ...
    async checkVerification() {
      ...
      if (this.getAppState.emailHash && this.getAppState.verified === true) {
        ...
        clearInterval(this.codeChecker);
        this.codeChecker = null;
        this.$ons.notification.alert('Your verification code is approved.').then((result) => {
          this.redirect('/');
        });
      }
    },

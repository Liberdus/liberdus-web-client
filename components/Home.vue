<template>
  <!-- <v-ons-page> -->
  <div>
    <notification :last-message="lastMessage" :last-tx="lastTx" />
    ...
    <div v-if="isUIReady" class="home-tab-container">
      ...
      <div class="total-balance">
        ...
        <p v-if="getAppState && !getAppState.emailHash" class="register-reminder">
      ...
      <div class="wallet-action-container" v-if="getAppState && getAppState.data.balance >= 0">
        <div class="wallet-action">
          ...
          <a-button type="primary" shape="round" size="large" icon="check" @click="$router.push('/wallet/send')">
            Send
          </a-button>
        </div>
        <div class="wallet-action">
          ...
          <a-button shape="round" size="large" icon="download" @click="$router.push('/wallet/receive')">
            Receive
          </a-button>
        </div>
      </div>
      <v-ons-list id="transaction-list">
        <v-ons-list-item v-for="transaction in transactions" :key="transaction.type + transaction.amount + transaction.timestamp">
...
export default {
      ...
      const processRawTransferTxs = (txList) =>
        map(txList, (tx) => {
          return {
            ...
            otherPersonAddress: tx.from.toLowerCase() === myAddress.toLowerCase() ? tx.to : tx.from,
      ...
      const processRawRewardTxs = (txList) =>
        map(txList, (tx) => {
          return {
            ...
            timestamp: tx.timestamp,
            otherPersonAddress: 'NETWORK',
            amount: this.networkParameters ? this.networkParameters.current.nodeRewardAmount : 1,
            fee: 0,
          };
        });
      ...
      const processProposalTxs = (txList) =>
        map(txList, (tx) => {
          return {
            ...
            otherPersonAddress: 'NETWORK',
            timestamp: tx.timestamp,
            amount: this.networkParameters ? this.networkParameters.current.proposalFee : 1,
          };
        });
      const processDevProposalTxs = (txList) =>
        map(txList, (tx) => {
          return {
            ...
            otherPersonAddress: 'NETWORK',
            timestamp: tx.timestamp,
            amount: this.networkParameters ? this.networkParameters.current.devProposalFee : 1,
          };
        });
  ...
  methods: {
    ...
    notifyToRegisterEmail() {
      this.$ons.notification.alert('Please register your email to earn some initial coins.');
    },
    getLastTxFromAPI() {
    ...
    getLatestMessageFromServer(processedState) {
      ...
      if (messageList.length > 0) {
        let sortedMessageList = messageList.sort((a, b) => b.timestamp - a.timestamp);
        return {
      ...
      try {
        ...
        for (let otherPersonPk in processed.data.chats) {
          ...
          const decryptedChatList = encryptedChatList.map((data) => {
            return utils.decryptMessage(data, otherPersonPk, self.getWallet.entry.keys.secretKey);
          });
    ...
    getActiveWindow(window, proposalType) {
      ...
      if (proposalType === 'economy') {
        if (now >= window.proposalWindow[0] && now < window.proposalWindow[1]) {
          return 'PROPOSAL';
        } else if (now >= window.votingWindow[0] && now < window.votingWindow[1]) {
          return 'VOTING';
        } else if (now >= window.graceWindow[0] && now < window.graceWindow[1]) {
          return 'GRACE';
        } else if (now >= window.applyWindow[0] && now < window.applyWindow[1]) {
          return 'APPLY';
        }
      } else if (proposalType === 'funding') {
        if (now >= window.devProposalWindow[0] && now < window.devProposalWindow[1]) {
          return 'PROPOSAL';
        } else if (now >= window.devVotingWindow[0] && now < window.devVotingWindow[1]) {
          return 'VOTING';
        } else if (now >= window.devGraceWindow[0] && now < window.devGraceWindow[1]) {
          return 'GRACE';
        } else if (now >= window.devApplyWindow[0] && now < window.devApplyWindow[1]) {
          return 'APPLY';
        }
      }
      return 'IDLE';
    ...
    async getWindowObj(proposalType) {
      try {
        let newNetworkParameters = await utils.queryParameters(`/home, getWindowObj`);
        if (!this.networkParameters) {
          this.networkParameters = newNetworkParameters;
        }
        const WINDOW_TYPE = proposalType === 'economy' ? 'windows' : 'devWindows';
    ...
    async refreshAppState() {
      ...
      if (self.getWallet && self.isUIReady) {
        ...
        let lastMessageFromServer = this.getLatestMessageFromServer(processedState);
    ...
    async refreshProposalList() {
      ...
      let networkParameters = await utils.queryParameters(`/home, refreshProposalList`);
      ...
      if (activeEconomyWindow === 'VOTING') {
        ...
        let economyProposalCount = JSON.parse(localStorage.getItem('economy_proposal_count')) || 0;
      ...
      let activeDevProposalList = allDevProposals.filter((proposal) => proposal.approved !== true && proposal.approved !== false);
      ...
      if (activeFundingWindow === 'VOTING') {
        ...
        let fundingProposalCount = JSON.parse(localStorage.getItem('funding_proposal_count')) || 0;
